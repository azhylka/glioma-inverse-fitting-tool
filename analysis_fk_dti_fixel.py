...
if evalButterfly:
    ...
    dtiPath = '/mnt/Drive2/andrey/cma_es_results/dti_results_butterfly/'
    fkPath = '/mnt/Drive2/andrey/cma_es_results/fk_results_butterfly/'
    fixelPath = '/mnt/Drive2/andrey/cma_es_results/fixel_results_butterfly/'
    
else:
    dtiPath = '/mnt/Drive2/andrey/cma_es_results/dti_results/'
    fkPath = '/mnt/Drive2/andrey/cma_es_results/fk_results/'
    fixelPath = '/mnt/Drive2/andrey/cma_es_results/fixel_results/'
...
patients_DTI, patients_FK, fk_solutions, dti_solutions = [], [], [], []
for patient_id in range(14, 3000):
    if patient_id in butterflyList:
        print(f"Skipping patient {patient_id} as it is in the butterfly list.")
        continue
    patstring = f"BraTS2021_{str(patient_id).zfill(5)}"
    ...
    if os.path.exists(dtiPath + patstring):
        ...
        for d in dirs:
            if "100_results.npy" in d or "101_results.npy" in d or "102_results.npy" in d or "50_results.npy" in d:
                dti_solutions.append(np.load(f"{dtiPath}{patstring}/{d}", allow_pickle=True).item())
                patients_DTI.append(patient_id)               
    ...
    if os.path.exists(fkPath + patstring):
        ...
        for d in dirs:
            if "100_results.npy" in d or "101_results.npy" in d or "102_results.npy" in d or "50_results.npy" in d:
                fk_solutions.append(np.load(f"{fkPath}{patstring}/{d}", allow_pickle=True).item())
                patients_FK.append(patient_id)

patients_FK = np.array(patients_FK).astype(str)
patients_DTI = np.array(patients_DTI).astype(str)
...
#%%
lossDTIs, diceFlairDTIs, diceT1cDTIs = getLossForListOfRuns(dti_solutions)
lossFKs, diceFlairFKs, diceT1cFKs = getLossForListOfRuns(fk_solutions)

#%%
sphericitysDTI, volumesDTI = getSphericity(patients_DTI)
sphericitysFK, volumesFK = getSphericity(patients_FK)

#%%
plt.scatter(patients_DTI, sphericitysDTI, label="DTI")
...
plt.scatter(patients_DTI, 1 - np.array(lossDTIs), label="DTI")
plt.scatter(patients_FK, 1 - np.array(lossFKs), label="FK")
plt.xlabel("Patient")
plt.ylabel("Volume Weighted Dice")
plt.legend()
#%% plot dice flair
plt.scatter(patients_DTI, diceFlairDTIs, label="DTI")
plt.scatter(patients_FK, diceFlairFKs, label="FK")
plt.xlabel("Patient")
plt.ylabel("Dice Flair")
plt.legend()

#%% plot dice T1c
plt.scatter(patients_DTI, diceT1cDTIs, label="DTI")
plt.scatter(patients_FK, diceT1cFKs, label="FK")
plt.xlabel("Patient")
plt.ylabel("Dice T1c")
plt.legend()
...
#%%% plot runtime
runtimeDTIs = getRuntimeforListOfRuns(dti_solutions)
runtimeFKs = getRuntimeforListOfRuns(fk_solutions)
plt.plot(patients_DTI, np.array(runtimeDTIs) / 60, label="DTI")
plt.plot(patients_FK, np.array(runtimeFKs) /60, label="FK")
plt.xlabel("Patient")
...
weigtedDiceDiff, pats, fk_loss, dti_loss = getDiffs(patients_FK, 1- np.array(lossFKs), patients_DTI, 1-np.array(lossDTIs))
...
flairDiceDiff, pats, fk_flair, dti_flair = getDiffs(patients_FK, diceFlairFKs, patients_DTI, diceFlairDTIs)
...
t1cDiceDiff, pats, fk_t1c, dti_t1c = getDiffs(patients_FK, diceT1cFKs, patients_DTI, diceT1cDTIs)
