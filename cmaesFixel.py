import sys
sys.path.append('/Users/azhylka/Projects/TumorGrowthToolkit')
from TumorGrowthToolkit.FK_FOD import FK_Fixel_Solver as fwdSolverFixel
from TumorGrowthToolkit.FK_FOD import FK_Fixel_n_Orig_Solver as fwdSolverFixelnOrig
...
class CmaesSolver():
    ...
    def logImges(self, tumor):
        ...
        brainmask = np.logical_or(self.gm, self.wm)

        # brainmask = zoom(brainmask, self.settings['resolution_factor'], order=1)
        # edema = zoom(self.edema, self.settings['resolution_factor'], order=1)
        # enhancing = zoom(self.enhancing, self.settings['resolution_factor'], order=1)
        # necrotic = zoom(self.necrotic, self.settings['resolution_factor'], order=1)
        tissue = brainmask*0.4 + self.edema *0.5 + self.enhancing * 0.7 + self.necrotic * 0.9
    ...
    def getLoss(self, x, gen):
        ...
        parameters = {
            ...
            'fixel_dir': self.settings['fixel_dir'],
        ...
        match self.settings['solverType']:
            ...
            case 'Fixel':
                parameters["diffusionTensors"] = None
                parameters["difffusionEllipsoidScaling"] = None
                parameters["diffusionTensorExponent"] = 1
                parameters["diffusionTensorLinear"] = 0
                parameters["gm"] = self.gm
                parameters["wm"] = self.wm
                fwdSolver = fwdSolverFixel
            case 'FixelnOrig':
                parameters["diffusionTensors"] = None
                parameters["difffusionEllipsoidScaling"] = None
                parameters["diffusionTensorExponent"] = 1
